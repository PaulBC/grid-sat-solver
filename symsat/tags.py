...
def tag_tuple(literals, tags):
  '''Apply tuple of tags to a tuple of literals.'''
  return tuple(x(tag) for x, tag in zip(literals, tags))

def all_tag_tuples(literals, tag_tuple_list):
  '''Apply tuple of tags to a list of tuple of literals.'''
  return [tag_tuple(literals, tags) for tags in tag_tuple_list]
