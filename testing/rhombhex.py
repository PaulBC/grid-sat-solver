...
def run_hextile(fileroot, hex_constraints, equivalence, xorig, yorig, cellsize, randassign):
  ...
  # add some extra constraints for mirror image 
  root2 = MooreGridNode((0, 0, 0), Tesselated(DiagonalFlipped(equivalence.rowsize)), PeriodicTimeAdjust(1, 0, 0))
  grid2 = build_grid(root2)
  clauses.extend(inflate_grid_template(hex_constraints, grid2, G.name))
...
'''
  ...
  if isinstance(equivalence, Tesselated) and isinstance(equivalence.tessellation, FaceCentered):
    adjust = 0 # 1
  for i in range(0, len(valuegrid[0]) - adjust):
    row = []
    for j in range(0, len(valuegrid[0][0]) - adjust):
      it, jt, _ = root.equivalence.to_equivalent(i, j)
      if True or i == j or (i != len(valuegrid[0]) - 1 and  j != 0):
  ...
  cells = []
  adjust = 0
  for i in range(-1, 2 * len(valuegrid[0]) - adjust):
    row = []
    for j in range(-1, 2 * len(valuegrid[0][0]) - adjust):
      it, jt, _ = root2.equivalence.to_equivalent(i, j)
      if True or i == j or (i != len(valuegrid[0]) - 1 and  j != 0):
        row.append(valuegrid[0][it][jt])
      else:
        row.append(None)
  ...
  #draw_hex_cells(xorig, yorig - 400, cells, cellsize)
...
if __name__ == "__main__":
  ...
  run_hextile(sys.argv[1], HEX_CONSTRAINTS, Tesselated(tessellation(size)), -250, 350, cellsize, randassign)
